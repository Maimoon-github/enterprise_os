...
1. Client-Server 1:1 Session Boundary:
   MCP Host -> 1:1 MCP Client Session -> 1:1 MCP Server Session -> Scoped Resources/Tools.
   Each client maintains an isolated, stateful connection with exactly one server.
   Servers operate in process/sandbox isolation and cannot access peer servers, unrestricted
   host conversation history, or global state.
2. Governed Tool Lifecycle:
   tools/list -> allowlist projection -> model tool-call intent -> host authorization ->
   validated tools/call -> server validation -> execution -> structured result/error -> audit.
...
4. Defense-in-Depth Validation & Credential Separation:
   Arguments validated at the host before dispatch and at the server boundary before execution.
   Upstream service credentials remain strictly server-side; client tokens are short-lived and audience-bound.
5. Ephemeral Sub-Agent Bindings:
   Tool/capability bindings allocated to sub-agents terminate immediately with the bounded subtask.
6. Auditability & Observability:
   Every invocation, clearance token, argument hash, and execution outcome is committed to an append-only audit trail.
"""
...
# ============================================================================
# 3. SCHEMA VALIDATOR (DEFENSE-IN-DEPTH HELPER)
# ============================================================================

class SchemaValidator:
    """Validates arguments against JSON Schema definitions at both Host and Server boundaries."""

    @staticmethod
    def validate(payload: Dict[str, Any], schema: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
...
# ============================================================================
# 4. MCP SERVER (ISOLATED CAPABILITY PROVIDER)
# ============================================================================
...
class MCPServer:
    ...
    async def handle_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if method == "initialize":
            ...
            # Defense-in-depth: Server-side argument validation before execution
            valid, err = SchemaValidator.validate(arguments, tool_entry.definition.inputSchema)
            if not valid:
                return {
                    "jsonrpc": "2.0",
                    "id": msg_id,
            ...
            try:
                # Execute tool passing isolated server-side credentials
...
# ============================================================================
# 5. MCP CLIENT (1:1 PROTOCOL GATEWAY)
# ============================================================================

class MCPClient:
    """
    Dedicated 1:1 client session connector for a single MCP Server.
    Handles message transport, protocol negotiation, timeouts, and error unwrapping.
    ...
    async def connect(self) -> None:
        ...
        raw_resp = await asyncio.wait_for(
            self.transport.receive_from_server(),
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any],
                        timeout_seconds: Optional[float] = None) -> CallToolResult:
        ...
        try:
            ...
            raise TimeoutError(f"Tool execution '{tool_name}' timed out after {effective_timeout}s")
...
# ============================================================================
# 6. MCP HOST & GOVERNED TOOL LIFECYCLE CONTROLLER
# ============================================================================
...
class EphemeralSubagentSession:
    """
    Ephemeral tool binding intended strictly for a sub-agent.
    Terminates cleanly with the bounded subtask lifecycle.
    """

    def __init__(self, session_id: str, subagent_id: str, host: MCPHost,
                 caller: CallerContext, task_grant: TaskCapabilityGrant,
                 permitted_tool_slice: Set[str]):
        self.session_id = session_id
        self.subagent_id = subagent_id
        self.host = host
        self.caller = caller
        self.task_grant = task_grant
        self.permitted_tool_slice = permitted_tool_slice
        self.is_terminated = False
        self.created_at = datetime.now(timezone.utc)

    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any],
                           approval_token: Optional[str] = None) -> CallToolResult:
        if self.is_terminated:
            raise RuntimeError(f"Ephemeral subagent session {self.session_id} has terminated.")
        return await self.host.invoke_governed_tool(
            caller=self.caller,
            task_grant=self.task_grant,
            tool_name=tool_name,
            arguments=arguments,
            approval_token=approval_token,
            subagent_scope_tools=self.permitted_tool_slice
        )

    def terminate(self) -> None:
        """Terminates ephemeral subagent tool session."""
        self.is_terminated = True
        self.permitted_tool_slice.clear()
        logger.info("Ephemeral subagent session %s for subagent %s terminated.",
                    self.session_id, self.subagent_id)
...
class MCPHost:
    ...
    def register_server_client(self, server_id: str, client: MCPClient,
        ...
        for t in server_tools:
            ...
            self._tool_definitions[t.name] = t
    ...
    def discover_effective_tools(self, caller: CallerContext,
        ...
        for tool_name in effective_names:
            ...
            if srv_id and srv_id in task_grant.allowed_servers:
                tool_defn = self._tool_definitions.get(tool_name)
                if tool_defn:
                    projected_tools.append(tool_defn)
    ...
    def create_ephemeral_subagent_session(self, subagent_id: str, caller: CallerContext,
                                          task_grant: TaskCapabilityGrant,
                                          permitted_tool_slice: Set[str]) -> EphemeralSubagentSession:
        """Provisions an ephemeral, attenuated tool-session for a sub-agent."""
        session_id = f"eph_{uuid.uuid4().hex[:8]}"
        attenuated_slice = permitted_tool_slice.intersection(task_grant.allowed_tools)
        return EphemeralSubagentSession(
            session_id=session_id,
            subagent_id=subagent_id,
            host=self,
            caller=caller,
            task_grant=task_grant,
            permitted_tool_slice=attenuated_slice
        )
    ...
    async def invoke_governed_tool(self, caller: CallerContext,
        ...
        is_high_impact = (tool_defn and tool_defn.is_high_impact) or (tool_name in self._high_impact_tools) or is_spend
        ...
        # 4. Host-Side Schema Pre-Validation (Defense-in-Depth)
        if tool_defn:
            valid, err = SchemaValidator.validate(arguments, tool_defn.inputSchema)
            if not valid:
                self._record_audit(audit_id, now_iso, caller, tool_name, server_id, arg_hash, "DENIED_INVALID_SCHEMA", "FAILED")
                raise ValueError(f"Host-side argument schema validation failed: {err}")

        client = self._clients[server_id]

        # 5. Dispatch via Dedicated 1:1 Client Session
        try:
